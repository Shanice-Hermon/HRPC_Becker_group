# -*- coding: utf-8 -*-
"""
Created on Thu Feb 12 21:02:19 2026
...
def generate_bedrmod(filename):
    ...
    df2 = df2.explode("mod")
    df2["name"] = df2["mod"].apply(lambda x: x[0])
    ...
    df3 = df2.groupby(["name", 
                       "chromStart", 
                       "chromEnd"]).agg(coverage = ("name", "size"),
    ...
    df_final = df3.loc[:, ["chrom",
                           ...
                           "itemRgb",
                           "coverage",
                           "frequency"]].sort_values(by = ["chromStart"]).reset_index(drop = True)
    
    return df_final

files = [
         "HRPC_5.8S_rRNA.csv",
         "HRPC_18S_rRNA_02.csv",
         "HRPC_28S_rRNA.csv"
         ]

dfs = [generate_bedrmod(f) for f in files]
